...
DTS_FILE="target/linux/airoha/dts/an7581-nokia_xg-040g-md.dts"
if [ -f "$DTS_FILE" ]; then
  # Check if patch already applied (idempotent)
  if ! grep -q "reg = <0x10210000 0x1000>, <0x10220000 0x1000>;" "$DTS_FILE"; then
    # Insert reg first, then reg-names to ensure correct order
    sed -i '/compatible = "airoha,en7581-cpufreq";/a \	reg = <0x10210000 0x1000>, <0x10220000 0x1000>;\n\	reg-names = "chip-scu", "mcucfg";' "$DTS_FILE"
